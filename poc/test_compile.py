...
print("Testing Torch Compile FP32...")
inputs = processor(text=txt, images=image, return_tensors="pt").to(device)

t0 = time.time()
for _ in range(3):
    model.generate(input_ids=inputs["input_ids"], pixel_values=inputs["pixel_values"], max_new_tokens=20, num_beams=1, early_stopping=False)
